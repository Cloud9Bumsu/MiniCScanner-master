...
enum tsymbol tnum[NO_KEYWORD] = {
	...
	tdouble,	tstring,	tfor,	tswitch,	tcase,	tdefault,	tcontinue,	tbreak
};
...
struct tokenType scanner(FILE *sourceFile)
{
    ...
    do {
        ...
        if (superLetter(ch)) { // identifier or keyword or char
            i = 0;
            do {
			...
			do { 
				if (ch == '.') {							// real number
					...
					dvalue=(double)token.value.num;
					ch = fgetc(sourceFile);
					do { 
						dvalue = 10 * dvalue + (double)(ch - '0');
						digits++;
						ch = fgetc(sourceFile);
					} while (isdigit(ch));
...
void writeToken(struct tokenType token, FILE *outputFile)
{
	...
	else if (token.number == tchar) {
		fprintf(outputFile, "im char Token %10s ( %3d, \'%c\' )\n", tokenName[token.number], token.number, token.value.ch);
	}
    else if (token.number == tstring){
        fprintf(outputFile, "im str Token %10s ( %3d, %s )\n", tokenName[token.number], token.number, token.value.str);
    }
	else {
